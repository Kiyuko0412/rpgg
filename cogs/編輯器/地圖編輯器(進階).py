...
class MapEditor(tk.Tk):
    ...
    def load_maps_data(self):
        try:
            with open(r'C:\Users\Administrator\Desktop\skill\cogs\maps_data.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        except (FileNotFoundError, json.JSONDecodeError) as e:
            messagebox.showerror("錯誤", f"無法讀取 maps_data.json: {e}")
            return {}

    def save_maps_data(self):
        try:
            with open(r'C:\Users\Administrator\Desktop\skill\cogs\maps_data.json', 'w', encoding='utf-8') as f:
    ...
    def create_widgets(self):
        ...
        ttk.Button(map_buttons_frame, text="全部儲存至檔案", command=self.save_maps_data).pack(fill="x", pady=(10,0))
    ...
    def save_current_map(self):
        ...
        try:
            ...
            messagebox.showinfo("成功", f"地圖 '{self.current_map_id}' 已在記憶體中更新。請記得 '全部儲存至檔案'。")
            self.populate_maps_list()
            if self.maps_tree.exists(self.current_map_id):
                self.maps_tree.selection_set(self.current_map_id)
                self.maps_tree.see(self.current_map_id)
